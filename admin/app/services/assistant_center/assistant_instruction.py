...
def render_assistant_instruction(params):
    ...
    if not target_assistant_rel:
        return next_console_response(error_status=True, error_message="无权限！", error_code=1002)
    instruction_prompt = Template(instruction.instruction_prompt)
...
def run_assistant_instruction(params):
    ...
    if not target_msg:
        app.logger.warning(f"RAG 消息不存在 not found")
    ...
    try:
        system_prompt_template = Template(assistant_instruction.instruction_system_prompt_template)
    ...
    try:
        user_prompt_template = Template(assistant_instruction.instruction_user_prompt_template)
    ...
    if assistant_instruction.instruction_result_template:
        instruction_result_template = Template(assistant_instruction.instruction_result_template)
