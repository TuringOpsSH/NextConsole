...
def add_session(params):
    ...
    db.session.commit()
    return next_console_response(result=new_session.to_dict())
...
def search_session(params):
    ...
    if not is_nc_admin:
        session_list = session_list.join(
            UserInfo,
            UserInfo.user_id == NextConsoleSession.user_id
        ).filter(
            UserInfo.user_company_id == admin_user.user_company_id
        )
    session_list = session_list.order_by(desc(NextConsoleSession.update_time))
    if fetch_all:
    ...
    for session in session_list:
        ...
        if app_code and app_code in app_info_dict:
            ...
            session["app_icon"] = ""
    # 补充用户信息
    all_user_ids = list(set([session["user_id"] for session in session_list if session["user_id"]]))
    all_user_info = UserInfo.query.filter(
        UserInfo.user_id.in_(all_user_ids)
    ).all()
    user_info_dict = {user.user_id: user.show_info() for user in all_user_info}
    for session in session_list:
        user_id = session["user_id"]
        if user_id in user_info_dict:
            session["user_info"] = user_info_dict[user_id]
        else:
            session["user_info"] = {}
...
def search_qa(params):
    ...
    搜索问答
    """
    admin_user_id = int(params.get("user_id"))
    session_id = params.get("session_id", [])
    qa_id = params.get("qa_id", [])
    ...
    admin_user = UserInfo.query.filter(
        UserInfo.user_id == admin_user_id,
        UserInfo.user_status == 1,
    ).first()
    is_nc_admin = check_has_role(admin_user_id, "next_console_admin")
    all_conditions = []
    if session_id:
    ...
    qa_list = NextConsoleQa.query.filter(*all_conditions)
    if not is_nc_admin:
        qa_list = qa_list.join(
            UserInfo,
            UserInfo.user_id == NextConsoleQa.user_id,
        ).filter(
            UserInfo.user_company_id == admin_user.user_company_id,
        ).with_entities(
            NextConsoleQa
        )
    qa_list = qa_list.order_by(desc(NextConsoleQa.create_time))
    if not fetch_all:
...
def add_messages(params, inner_call=False):
    ...
    if assistant_id and (assistant_id > 0 or assistant_id != -10086):
        target_session = NextConsoleSession.query.filter_by(
            id=session_id,
            user_id=user_id
        ).first()
...
def update_messages(params):
    ...
    if msg_remark and msg_remark != target_msg.msg_remark:
