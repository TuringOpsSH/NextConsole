...
def update_share_resource_access_list(params):
    ...
    # 检查用户是否有权限去变更资源的访问列表
    manage_access = False
    if check_user_manage_access_to_resource({'user': target_user, 'resource': target_resource, 'access_type': 'manage'}):
        manage_access = True
...
def get_share_resource_meta(params):
    ...
    t6 = time.time()
    # 新增权限信息
    result["access_list"] = get_user_access_to_resource({
        "user": target_user,
        "resource": target_resource
    })
    t7 = time.time()
...
def search_share_resource_by_keyword(params):
    ...
    # 获取资源对应的作者信息
    all_author_id = list(set([resource.get("user_id") for resource in new_res]))
    all_author = UserInfo.query.filter(
        UserInfo.user_id.in_(all_author_id),
        UserInfo.user_status == 1
    ).all()
