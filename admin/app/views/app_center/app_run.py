...
@app.route("/next_console_admin/app_center/app_run/init_session", methods=["POST"])
@jwt_required()
def get_app_session():
    """
    获取应用详情
...
@app.route('/next_console_admin/app_center/app_run/v2/chat/completions', methods=["GET", "POST", "OPTIONS"])
@app.route('/next_console_admin/app_center/app_run/v2/chat/completions', methods=["GET", "POST", "OPTIONS"])
@jwt_required()
@cross_origin()  # 允许跨域访问
def app_add_message():
...
@app.route("/next_console_admin/app_center/app_run/debug_info", methods=["POST"])
@jwt_required()
def get_app_workflow_debug_info():
    """
    获取应用工作流调试信息
    """
    user_id = get_jwt_identity()
