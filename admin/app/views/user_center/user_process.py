...
@app.route("/next_console_admin/get_support_area", methods=["GET", "POST"])
def get_support_area():
    return get_support_area_data()


@app.route("/next_console_admin/check_register_email", methods=["GET", "POST"])
...
@app.route('/next_console_admin/register', methods=['POST'])
def register():
    """
    注册用户、初始化、发送确认邮件
    :return:
...
@app.route('/next_console_admin/confirm_email', methods=['GET', 'POST'])
...
@app.route('/next_console_admin/resend_confirm_email', methods=['GET', 'POST'])
...
@app.route('/next_console_admin/login_by_password', methods=['POST'])
...
@app.route("/next_console_admin/reset_account_password", methods=["POST"])
...
@app.route("/next_console_admin/reset_password_code/valid", methods=["POST"])
...
@app.route('/next_console_admin/user_center/users/get', methods=['POST'])
@jwt_required()
def user_get():
    """
    获取用户详情
...
@app.route('/next_console_admin/user_center/users/update', methods=['POST'])
@jwt_required()
def user_update():
...
@app.route('/next_console_admin/user_center/users/avatar/update', methods=['POST'])
@jwt_required()
...
@app.route('/next_console_admin/user_center/users/close', methods=['POST'])
@jwt_required()
def user_close():
    params = request.get_json()
    user_id = get_jwt_identity()
...
@app.route("/next_console_admin/reset_new_email", methods=["POST"])
@jwt_required()
def reset_new_email():
    data = request.get_json()
    user_id = get_jwt_identity()
...
@app.route("/next_console_admin/reset_email_code/valid", methods=["POST"])
@jwt_required()
...
@app.route('/next_console_admin/user_center/generate_text_code', methods=['POST'])
def send_text_code():
    """
    根据请求发送验证码
    :return:
...
@app.route('/next_console_admin/login_by_code', methods=['POST'])
def login_by_code():
    data = request.get_json()
    user_account = data.get('user_account')
...
@app.route("/next_console_admin/wx_register", methods=["POST"])
def wx_register():
    data = request.get_json()
    code = data.get("code")
    if not code:
...
@app.route("/next_console_admin/user_center/bind_new_phone", methods=["POST"])
@jwt_required()
def bind_new_phone():
    """
    绑定新手机
...
@app.route("/next_console_admin/user_center/valid_new_phone", methods=["POST"])
@jwt_required()
def valid_new_phone():
    """
    绑定新手机
