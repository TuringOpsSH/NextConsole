  ...
  <div class="form-main">
    <div v-if="props.isParent && !localReadOnly && !localSchema?.attrFixed" class="root-button">
      <el-tooltip content="添加变量" placement="top" effect="light">
        <el-icon class="icon-button" @click="addSubPrams('root', $event)">
          <Plus />
        </el-icon>
      </el-tooltip>
    </div>
          ...
          <div class="attr-require">
            ...
            <el-popover trigger="click" title="枚举值" width="300px" placement="left">
              <template #reference>
                <el-icon
                  v-if="
                    !['object', 'array', 'file', 'file-list', 'null', 'boolean'].includes(
        ...
        <el-col v-if="localValueDefile" :span="6">
          <div class="attr-type">
            <RefSelect
              ...
              :type-fixed="localSchema?.properties?.[key]?.typeFixed"
        ...
        <el-col v-if="!localReadOnly" :span="localValueDefile ? 4 : 6">
          <div v-if="!localReadOnly" class="std-middle-box" style="gap: 6px">
            <el-tooltip content="添加子变量" placement="top" effect="light">
              <el-icon
                v-show="
                  localSchema?.properties?.[key]?.type == 'object' &&
                  !localSchema?.properties?.[key]?.ref &&
                  !localSchema?.properties?.[key]?.attrFixed
                "
                class="icon-button"
    ...
    <div v-else-if="localSchema?.items" class="form-item-body">
      <el-row>
        ...
        <el-col :span="7">
          <el-select
            v-model="localSchema.items.typeName"
            :disabled="localReadOnly || localSchema?.items?.attrFixed"
  ...
  <el-dialog v-model="showFileEnumFlag" title="可上传的文件类型">
    <el-form style="margin-top: 20px">
      <el-form-item label="文件类型" label-position="top">
        <el-select
          v-if="localSchema.properties?.[currentKey]?.enum"
