...
const currentLLM = reactive<ILLMInstance>({
  create_time: '',
  llm_code: '',
  llm_desc: '',
  llm_icon: '',
...
<template>
  <el-container>
      ...
      <el-tabs v-model="currentPage" @tab-click="handleTabClick">
        <el-tab-pane v-loading="LLMListLoading" label="模型管理" name="llm">
          <div id="model-list-area">
            ...
            <div id="model-list-area">
              <div v-for="llm in currentLLMList" :key="llm.llm_code" class="llm-card">
                <div class="card-head">
    ...
    <el-dialog v-model="showNewLLMForm" :fullscreen="true" title="新建模型实例">
      <div class="std-middle-box">
        <el-form ref="newLLMFormRef" :model="newLLMForm" :rules="newLLMRules" class="new-llm" label-position="top">
          ...
          <el-form-item label="模型API访问密钥" prop="llm_api_access_key">
            <el-input
              v-model="newLLMForm.llm_api_access_key"
          ...
          <el-form-item v-show="newLLMForm.llm_is_proxy" label="代理地址" prop="llm_proxy_url">
            <el-input v-model="newLLMForm.llm_proxy_url" placeholder="请输入模型代理访问地址" clearable />
          </el-form-item>
          ...
          <el-form-item label="模型供应商" prop="llm_company">
            <el-input v-model="newLLMForm.llm_company" placeholder="请输入模型供应商名称" clearable />
          </el-form-item>
      ...
      <template #footer>
        <div class="std-middle-box">
          <el-button round @click="showNewLLMForm = false"> 取消 </el-button>
          <el-button type="primary" round @click="commitNewLLMInstance"> 确认 </el-button>
        </div>
      </template>
    </el-dialog>
    <el-dialog v-model="showDeleteLLMConfirm" title="删除模型实例">
      <el-result
        :title="'删除选中模型实例:' + currentLLM.llm_name + '?'"
        icon="error"
    ...
    <el-dialog v-model="showUpdateLLMForm" :fullscreen="true" title="模型实例详情">
      <div class="std-middle-box">
        ...
        >
          ...
          <el-form-item label="模型API访问密钥" prop="llm_api_access_key">
            <el-input
              v-model="currentLLM.llm_api_access_key"
              placeholder="请输入模型API访问密钥"
              clearable
              type="password"
              show-password
          ...
          <el-form-item v-show="currentLLM.llm_is_proxy" label="代理地址" prop="llm_proxy_url">
            <el-input v-model="currentLLM.llm_proxy_url" placeholder="请输入模型代理访问地址" clearable />
          </el-form-item>
          <el-form-item label="共享使用" prop="llm_is_public">
