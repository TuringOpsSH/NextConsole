...
import { nodeUpdate, workflowUpdate, workflowSearch } from '@/api/app-center-api';
import { type CSSProperties, onMounted, reactive, ref, nextTick } from 'vue';
import JsonSchemaForm from '@/components/app-center/JsonSchemaForm.vue';
import TemplateEditor from '@/components/app-center/TemplateEditor.vue';
import { llmInstanceSearch } from '@/api/config-center';
import { LLMInstance } from '@/types/config-center';
import { currentApp } from '@/components/app-center/ts/app-detail';
import ResourcesSearch from '@/components/app-center/appPreview/ResourcesSearch.vue';
import {
  CurrentEditFlow,
  currentNodeDetail,
  deleteCurrentNode,
...
import RefSelect from '@/components/app-center/refSelect.vue';
...
async function handleFileSplitterModeChange(val:string) {
  ...
  if (val == 'single') {
    currentNodeDetail.value.node_input_params_json_schema.properties['content'] = {
      type: 'string',
      typeName: 'string',
      value: '',
      ref: '',
    ...
    delete currentNodeDetail.value.node_input_params_json_schema.properties['contents'];
    currentNodeDetail.value.node_input_params_json_schema.ncOrders[0] = 'content';

  }
  else if (val == 'list') {
    currentNodeDetail.value.node_input_params_json_schema.properties['contents'] = {
      type: 'array',
    ...
    delete currentNodeDetail.value.node_input_params_json_schema.properties['content'];
    currentNodeDetail.value.node_input_params_json_schema.ncOrders[0] = 'contents';
        ...
        <div v-if="currentNodeDetail?.node_type == 'file_splitter'" class="config-item">
          <div class="config-area">
            <el-form :model="currentNodeDetail" label-position="top">
              <el-form-item prop="mode" label="模式" style="padding: 0 12px">
                <el-radio-group
                    v-model="currentNodeDetail.node_file_splitter_config.mode"
              ...
              <el-form-item prop="method" label="分块基础大小" style="padding: 0 12px">
                <el-input-number v-model="currentNodeDetail.node_file_splitter_config.chunk_size"
                                :min="1"
                                :max="60000" placeholder="请选择分块基础大小" :precision="0"
                                 style="width: 100%"
                                @change="updateNodeFileSplitterConfig"
                />
              </el-form-item>
            ...
            <el-form :model="currentNodeDetail" label-position="top" style="padding: 12px">
              <el-form-item label="运行方式" prop="node_run_model">
                <el-radio-group v-model="currentNodeDetail.node_run_model_config.node_run_model"
                  ...
                  <el-radio-button value="parallel" :disabled="currentNodeDetail.node_type != 'llm'">并行</el-radio-button>
