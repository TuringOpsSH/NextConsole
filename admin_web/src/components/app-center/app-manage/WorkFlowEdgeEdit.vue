...
import { ref } from 'vue';
import { workflowUpdate } from '@/api/app-center-api';
...
async function updateEdgeConfig() {
  const currentEdge = workflowStore.graphWrapper.getCellById(workflowStore.currentEdgeDetail.edge_code);
  currentEdge.updateData({
    edge_type: workflowStore.currentEdgeDetail.edge_type,
    edge_condition_type: workflowStore.currentEdgeDetail.edge_condition_type,
    edge_conditions: workflowStore.currentEdgeDetail.edge_conditions
  });
  currentEdge.prop('labels', [workflowStore.currentEdgeDetail.edge_type]);
...
async function addEdgeCondition() {
  const currentEdge = workflowStore.graphWrapper.getCellById(workflowStore.currentEdgeDetail.edge_code);
  if (!workflowStore.currentEdgeDetail?.edge_conditions) {
    workflowStore.currentEdgeDetail.edge_conditions = [];
  }
  workflowStore.currentEdgeDetail.edge_conditions.push({
    src_node: '',
    operator: '==',
    tgt_node: ''
  });
...
async function deleteEdgeCondition(idx: number) {
  const currentEdge = workflowStore.graphWrapper.getCellById(workflowStore.currentEdgeDetail.edge_code);
  if (workflowStore.currentEdgeDetail?.edge_conditions) {
    workflowStore.currentEdgeDetail.edge_conditions.splice(idx, 1);
    currentEdge.updateData({
      edge_conditions: workflowStore.currentEdgeDetail.edge_conditions
    });
    const graphData = workflowStore.graphWrapper.toJSON();
...
async function updateEdgeCondition(idx: number, type: string, newValue) {
  const currentEdge = workflowStore.graphWrapper.getCellById(workflowStore.currentEdgeDetail.edge_code);
  if (workflowStore.currentEdgeDetail?.edge_conditions) {
    workflowStore.currentEdgeDetail.edge_conditions[idx][type] = newValue;
    currentEdge.updateData({
      edge_conditions: workflowStore.currentEdgeDetail.edge_conditions
    });
    const graphData = workflowStore.graphWrapper.toJSON();
...
<template>
  <div
    v-show="workflowStore.showEdgeFlag"
    id="agent-edge-detail-box"
    ref="edgeDetailRef"
    v-loading="workflowStore.loadingEdgeInfo"
  ...
  >
    <el-scrollbar style="width: 100%">
      <div id="agent-edge-detail">
          ...
          <div id="edge-detail-head-top">
            <div id="edge-detail-head-left">
              <div class="std-middle-box">
                <el-image class="agent-edge-icon" :src="workflowStore.currentEdgeDetail?.edge_icon" />
              </div>
              <div class="std-middle-box">
                <el-text class="agent-node-name"> {{ workflowStore.currentEdgeDetail?.edge_name }} </el-text>
              </div>
            </div>
          </div>
          <div class="std-left-box">
            <el-text> {{ workflowStore.currentEdgeDetail?.edge_desc }}</el-text>
          </div>
        </div>
        <div class="config-item">
          <div v-show="workflowStore.currentEdgeDetail" class="config-area">
            <el-form :model="workflowStore.currentEdgeDetail" label-position="top" style="padding: 12px">
              <el-form-item prop="edge_type" label="关系种类">
                <el-radio-group v-model="workflowStore.currentEdgeDetail.edge_type" @change="updateEdgeConfig">
              ...
              <el-form-item prop="edge_condition_type" label="条件连接类型">
                <el-radio-group
                  v-model="workflowStore.currentEdgeDetail.edge_condition_type"
                  @change="updateEdgeConfig"
                >
              ...
              <el-form-item prop="edge_conditions" label="条件列表">
                <div class="condition-item-box">
                  <div
                    v-for="(item, idx) in workflowStore.currentEdgeDetail?.edge_conditions"
                    :key="idx"
                    class="condition-item"
                  >
                    <div class="condition-src-item">
                      <RefSelect
                        :up-stream-nodes="workflowStore.currentEdgeDetail.node_upstream"
                        :ref-value="workflowStore.currentEdgeDetail.edge_conditions[idx].src_node"
                        @update:ref="result => updateEdgeCondition(idx, 'src_node', result)"
                      />
                    </div>
                    <div class="condition-operator-box">
                      <el-select
                        v-model="workflowStore.currentEdgeDetail.edge_conditions[idx].operator"
                        @change="updateEdgeConfig"
                      >
                    ...
                    <div
                      v-show="
                        !['is null', 'not null'].includes(
                          workflowStore.currentEdgeDetail.edge_conditions[idx]?.operator
                        )
                      "
                    ...
                    >
                      <RefSelect
                        :up-stream-nodes="workflowStore.currentEdgeDetail.node_upstream"
                        :ref-value="workflowStore.currentEdgeDetail.edge_conditions[idx].tgt_node"
        ...
        <div v-if="workflowStore.currentEdgeDetail?.routerName" class="config-item">
          <el-text>边路由配置</el-text>
          <el-select v-model="workflowStore.currentEdgeDetail.routerName" @change="changeEdgeRouter">
