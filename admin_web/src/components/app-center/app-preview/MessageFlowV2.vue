...
import { getDebugInfo } from '@/api/app-center-api';
import {
...
const referenceDrawerWidth = ref(window.innerWidth < 768 ? '60vw' : '60vw');
...
function openReference(data: IReferenceItem | null) {
  if (data.source_type == 'webpage') {
    // 新标签页打开
    window.open(data.resource_id as string);
  } else {
    // 跳转资源预览页面
...
async function getQADebugInfo(item) {
  ...
  if (!res.error_status) {
    currentDebugInfo.value = res.result;
    showDebugInfoFlag.value = true;
  }
}
  ...
  >
    <el-container>
      <el-main class="msg-main" @dragover.prevent="handleDragOver">
                        ...
                        <template v-else-if="attachment.resource_type == 'document'">
                          ...
                          <div v-else class="attachment-item" @click="downloadAttachment(attachment)">
                            <div class="std-middle-box">
                              <el-image
                                :src="getResourceIcon({ resource_icon: attachment.resource_icon } as IResourceItem)"
                                class="attachment-item-img"
                              />
                            </div>
                ...
                <div class="msg-flow-answer-box" @mouseenter="item.show_button_answer_area = true">
                  ...
                  <div class="msg-flow-answer-content">
                    ...
                    <div class="msg-flow-answer-inner" :class="{ 'msg-flow-answer-inner-short': item?.short_answer }">
                      <div
                        v-for="(sub_finish_msg, subIdx) in item.qa_value.answer[item.qa_value.question[0]?.msg_id]"
                        :key="subIdx"
                        style="width: 100%"
                      >
        ...
        <el-dialog v-model="showFullTaskResultFlag" title="完整结果">
          <el-scrollbar>
