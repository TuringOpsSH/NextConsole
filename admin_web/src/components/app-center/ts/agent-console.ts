...
import { addAppMessage } from '@/api/app-center-api';
import { search_messages, update_session } from '@/api/next-console';
import {
  addAgentAppQa,
  AgentAppMsgFlow,
  CurrentAgent,
...
import { user_info } from '@/components/user-center/user';
import { LLMInstance } from '@/types/config-center';
...
export async function askAgentQuestion() {
  ...
  const data = await addAppMessage(question_params, null);
  ...
  try {
    const data = await add_app_messages(params, msg_abort_controller.signal);
    // 处理流式响应来更新消息
    ...
    while (!chunk.done) {
      ...
      AgentAppMsgFlow.value[new_qa_item_idx].qa_value.answer[msg_parent_id][0].msg_content += new TextDecoder(
        'utf-8'
      ).decode(chunk.value);
...
export function get_running_progress(target_question: running_question_meta) {
  try {
    const msg_parent_id = msg_flow.value[target_question.qa_item_idx].qa_value.question[0].msg_id;
