...
import { api, appAdd, appDelete, appImport, appSearch } from '@/api/appCenterApi';
import {currentApp, currentFlowList, initCurrentApp} from '@/components/appCenter/ts/app-detail';
...
async function beginEdit(app: IAppMetaInfo) {
  ...
  if (currentApp.code !== app.app_code) {
    await initCurrentApp(app.appCode);
  }
}
...
<template>
  <el-container>
      ...
      <div id="app-header-1">
        ...
        <div>
          <el-input
              ...
              placeholder="搜索应用"
              clearable
              @keydown.enter.prevent="searchAppList"
              @clear="searchAppList"
          />
        </div>
      </div>
    ...
    <el-main style="height: calc(100vh - 160px); padding: 0">
      <el-scrollbar>
        <div id="app-main">
          <div v-for="app in appList" :key="app.id" class="app-item-card">
