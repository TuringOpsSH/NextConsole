...
import { get_all_company, searchDepartments } from '@/api/dashboard';
import { admin_search_user, twadmin_search_user } from '@/api/user_manage';
...
async function SearchNewCompany() {
  ...
  const searchRes = await get_all_company({});
  if (!searchRes.error_status) {
...
<template>
  <el-container>
    <el-header height="60px">
      <div id="app-header-1">
        <h3>发布详情:{{ currentAppCode }}</h3>
      </div>
    </el-header>
    <el-main style="height: calc(100vh - 130px)">
      <el-tabs :model-value="currentViewType" @tab-change="handleChangeModel">
        ...
        <el-tab-pane label="运行情况" name="running">
          ...
          <el-scrollbar>
            <div class="running-status-area">
              <div id="app-user" class="running-graph"></div>
              <div id="app-session-cnt" class="running-graph"></div>
              <div id="app-qa-cnt" class="running-graph"></div>
              <div id="app-attachment-cnt" class="running-graph"></div>
              <div id="app-attachment" class="running-graph"></div>
              <div id="app-token" class="running-graph"></div>
              <div id="app-time" class="running-graph"></div>
              <div id="app-success-rate" class="running-graph"></div>
            </div>
          </el-scrollbar>
        </el-tab-pane>
        <el-tab-pane label="授权管理" name="author" @tab-change="handleChangeModel">
          <el-tabs :model-value="currentAccessObject" tab-position="left" @tab-change="handleAccessObjectChange">
              ...
              <div class="access-head-area">
                <div class="access-head-left">
                  <el-form-item label="授权给全部用户" label-position="left" style="margin-bottom: 0">
                    <el-switch
                      v-if="userInfo?.user_role?.includes('next_console_admin')"
            ...
            <el-tab-pane v-if="userInfo?.user_role?.includes('next_console_admin')" label="企业授权" name="company">
              <div class="access-head-area">
                <div class="access-head-left">
                  <el-form-item style="margin-bottom: 0">
                    <el-input
