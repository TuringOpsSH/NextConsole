...
import {ISearchByKeywordsParams, ResourceItem} from '@/types/resource_type';
import {
...
export async function switch_resource_layout(target_model: string = null) {
  ...
  // await refreshData();
  if (!target_model) {
    if (resourceViewModel.value === 'list') {
      resourceViewModel.value = 'card';
    } else {
      resourceViewModel.value = 'list';
    }
  } else {
    resourceViewModel.value = target_model;
  }
  // 更新至url
  router.push({
    ...
    query: {
      ...
      view_model: resourceViewModel.value
    }
  });
}
