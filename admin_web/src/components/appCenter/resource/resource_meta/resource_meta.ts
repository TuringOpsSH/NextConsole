...
export const show_rebuild_button = ref(false)
export async function turn_on_resource_meta(resource_id:number, resource_status:string='正常'){
    ...
    if (!res.error_status){
        ...
        for (let rag_status of choose_resource_meta.rag_status){
            // @ts-ignore
        ...
        show_rebuild_button.value = !all_success;
        if (!choose_resource_meta.rag_status?.length){
            show_rebuild_button.value = true
        }
    }
    loading_meta.value = false
}
...
export async function switch_resource_edit_model(){
    ...
    // 检查是否有编辑和修改权限
    if (choose_resource_meta.user_id == user_info.value.user_id || choose_resource_meta?.access_list?.includes('管理')
    ){
...
export async function update_choose_resource_meta(){
    ...
    let res = await update_resource_object(params)
    if (!res.error_status){
...
export async function before_leave_check(done){
    ...
    if (!res.error_status){
        ...
        let all_history_tag_ids = []
        for (let item of res.result.resource_tags){
            all_history_tag_ids.push(item.id)
        }
        let all_now_tag_ids = []
        for (let item of choose_resource_meta.resource_tags){
...
export async function cancel_resource_edit_model(){
    let res = await get_resource_object({
        resource_id: choose_resource_meta.id
    })
    if (!res.error_status){
        choose_resource_meta.resource_name = res.result.resource_name
