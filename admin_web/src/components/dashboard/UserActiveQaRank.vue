      ...
      <ul class="ranking-list" :style="{ height: areaHeight + 'px' }">
        <li v-for="(user, idx) in users" :key="user.id" :class="['ranking-item', { top3: idx < 3 }]">
          ...
          <div class="info">
            ...
            <span class="days">问答 {{ user?.qa_count }} 次</span>
          </div>
          <!-- 徽章（仅前三名） -->
...
async function getUserQaRank(beginTime: string, companyId: number) {
  const params = {
    index_name: 'user_rank_qa',
  ...
  if (!res.error_status) {
    users.value = res.result.qa_rank;
  }
}
watch(
  () => props.height,
