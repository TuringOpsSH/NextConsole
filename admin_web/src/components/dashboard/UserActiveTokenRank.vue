      ...
      <ul class="ranking-list" :style="{ height: areaHeight + 'px' }">
        <li v-for="(user, idx) in users" :key="user.id" :class="['ranking-item', { top3: idx < 3 }]">
          ...
          <div class="info">
            ...
            <span class="days">消耗Token {{ user?.token_count }} </span>
          </div>
          <!-- 徽章（仅前三名） -->
...
async function getUserTokenRank(beginTime: string, companyId: number) {
  const params = {
    index_name: 'user_rank_token',
  ...
  if (!res.error_status) {
    users.value = res.result.token_rank;
  }
}
watch(
  () => props.height,
