...
const emits = defineEmits(['update:model', 'refresh']);
...
async function updateUserResult(response) {
  // 在这里处理服务器返回的结果
  loading.value = false;
...
<template>
  <el-dialog :ref="dialogRef" v-model="localModel" title="添加用户" @closed="handleDialogClosed">
    <div v-loading="loading" class="next-console-div" style="flex-direction: column" element-loading-text="努力导入中">
        ...
        <el-upload
          ...
          :action="
            userInfoStore.userInfo.user_role.includes('next_console_admin')
              ? api.twadmin_create_user_by_excel
              : api.admin_create_user_by_excel
          "
          with-credentials
      ...
      <div v-show="userExcelFile?.length > 0" class="next-console-div-upload">
        <div>
      ...
      <div class="user-add-form-header">
        <el-button
          color="#1570EF"
          style="border: 1px solid #1570ef; width: 100%"
