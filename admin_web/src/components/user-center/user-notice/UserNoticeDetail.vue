...
} from '@/components/user-center/user-notice/user_notice_detail';
        ...
        <div id="detail-header-right">
          <div>
            <el-button
              ...
              round
              @click="
                router.push({ name: 'user_notification_list' });
          ...
          <div v-if="current_notice_task.task_status == '新建中'">
            <el-button type="primary" round @click="update_current_task"> 保存 </el-button>
          </div>
          <div v-if="current_notice_task.task_status == '新建中'">
            <el-button type="success" round @click="start_notice_task"> 启动 </el-button>
          </div>
          <div v-else-if="current_notice_task.task_status == '执行中'">
            <el-button type="info" round @click="pause_notice_task"> 暂停 </el-button>
          </div>
          <div v-else-if="current_notice_task.task_status == '已暂停'">
            <el-button type="info" round @click="resume_notice_task"> 恢复 </el-button>
          </div>
          <div v-if="!['新建中', '已终止', '已完成'].includes(current_notice_task.task_status)">
            <el-button type="warning" round @click="stop_notice_task"> 终止 </el-button>
          </div>
          <div v-if="['新建中', '已终止', '已完成'].includes(current_notice_task.task_status)">
        ...
        <div id="notice-detail-area" v-loading="task_loading" :element-loading-text="loading_text">
          <el-form label-position="top" inline style="max-width: 925px">
            ...
            <el-form-item label="任务进度" style="width: 100%">
              <el-progress
                v-if="current_notice_task.task_instance_failed"
                :percentage="current_notice_task.task_progress"
                status="exception"
                :text-inside="true"
                :stroke-width="18"
                style="width: 100%"
              />
              <el-progress
                v-else-if="current_notice_task.task_status === '已完成'"
                :percentage="current_notice_task.task_progress"
                status="success"
                :text-inside="true"
                :stroke-width="18"
              ...
              <el-progress
                v-else-if="current_notice_task.task_status === '已暂停' || current_notice_task.task_status === '已终止'"
                :percentage="current_notice_task.task_progress"
                status="warning"
                :text-inside="true"
                :stroke-width="18"
          ...
          >
            ...
            <el-form-item label="立刻运行" prop="run_now">
              <el-checkbox
                v-model="current_notice_task.run_now"
                :disabled="current_notice_task.task_status != '新建中'"
              />
            </el-form-item>
            <el-form-item v-if="!current_notice_task.run_now" label="计划开始时间" prop="plan_begin_time">
              <el-date-picker
            ...
            <el-form-item v-if="!current_notice_task.run_now" label="计划完成时间" prop="plan_finish_time">
              <el-date-picker
            ...
            <el-form-item label="任务事务大小" prop="task_instance_batch_size">
              <el-input-number
                v-model="current_notice_task.task_instance_batch_size"
                :min="1"
                :max="1000"
                :step="10"
                :disabled="current_notice_task.task_status != '新建中'"
              ...
              <el-row style="width: 100%">
                <el-col :span="10">
                  <div class="std-middle-box">
                    <el-input
                      v-model="current_notice_task.notice_template"
                      type="textarea"
                      :rows="12"
                      :disabled="current_notice_task.task_status != '新建中'"
                      @blur="check_html_valid"
                    />
                  </div>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="10">
                  <div id="preview-area">
                    <div v-if="current_notice_template_valid" v-html="current_notice_task.notice_template" />
                    <div v-else>
            ...
            <el-form-item label="通知范围(取并集)" prop="notice_params">
              <div id="notice-area">
                ...
                <el-select
                  v-show="area_shortcut.includes('target_company')"
                  v-model="area_target_companies"
                  multiple
                  ...
                  value-key="id"
                  :max-collapse-tags="3"
                  @change="handle_notice_company_change"
                >
                  <el-option
                    v-for="item in area_company_options"
                    :key="item.id"
                ...
                <el-select
                  v-show="area_shortcut.includes('target_department')"
                  v-model="area_target_departments"
                  multiple
                  placeholder="请搜索后选择部门"
                  :disabled="current_notice_task.task_status != '新建中'"
                  collapse-tags
                  clearable
                  :remote-method="search_department"
                  remote
                  filterable
                  value-key="id"
                  :max-collapse-tags="3"
                  @change="handle_notice_department_change"
                >
                  <el-option
                    v-for="item in area_department_options"
                    :key="item.id"
                ...
                <el-select
                  v-show="area_shortcut.includes('target_user')"
                  v-model="area_target_users"
                  multiple
                  ...
                  :max-collapse-tags="3"
                  value-key="user_id"
                  @change="handle_notice_user_change"
                >
                  <el-option
                    v-for="item in area_user_options"
                    :key="item.user_id"
