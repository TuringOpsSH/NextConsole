; (function  (input, params){
    ...
    var result = {
        _sid: "",
        desc: "查看当前状态产生的InnoDB锁，仅在有锁等待时有结果输出",
        effect: "响应时间过长、无法完成当前事务",
        solution: "找到锁表的事务，分析锁表原因，进行优化",
        level: "正常",
        id: "mysql.view_the_innodb_lock_generated_by_the_current_state_and_the_result_is_output_only_when_there_is_a",
        name: "InnoDB锁",
        mixraw: [],
        raw: [],
        actual: "",
    ...
    result.raw = input
    if (input[0].indexOf("ERROR ") == -1) {
        result.level = "中风险"
        result.expected = "不存在InnoDB锁"
