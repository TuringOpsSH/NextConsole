; (function (input, params) {
    ...
    for (i = 0; i < raw.length; i++) {
        ...
        // fields["name"] //string
        // fields["value"] //string
        if (fields["name"] == "Archiving Enabled") {
        ...
        else if (fields["name"] == "Using SPFILE") {
            if (fields["value"] != "YES") {
        ...
        else if (fields["name"] == "Redo logs being multiplexed") {
            if (fields["value"] != "YES") {
                mixlevel = "中风险"; if (result.level != "高风险") { result.level = "中风险"; }
        ...
        else if (fields["name"] == "Redo logs the same size") {
            if (fields["value"] != "YES") {
                mixlevel = "中风险"; if (result.level != "高风险") { result.level = "中风险"; }
        ...
        else if (fields["name"] == "Number of Control Files") {
            if (fields["value"] == "1") {
                mixlevel = "中风险"; if (result.level != "高风险") { result.level = "中风险"; }
        ...
        else if (fields["name"] == "Min Redo Log Size(MB)") {
            minredo = Number(fields["value"])
        }
        else if (fields["name"] == "SGA Granul Size(MB)") {
    ...
    if (sgags > minredo) {
        result.mixraw.push({
            name: "SGA Granul Size(MB) > Min Redo Log Size(MB)",
            value: "YES",
            mixlevel: "高风险"
        })
