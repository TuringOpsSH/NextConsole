...
_netstat(){
/usr/bin/netstat -in | grep -v "lo0" | grep -v "link#" | grep  -v "^Name" | awk '{print $1",",$2",",$3",",$(NF-4)",",$(NF-3)",",$(NF-2)",",$(NF-1)",",$(NF)}'
}
...
multitenant=`${sqlpluscmd} -S "/as sysdba" <<EOF 2>/dev/null
set heading off feedback off pagesize 0 linesize 200;
set verify off echo off termout off trimout on trimspool on ;
set time off timing off SQLPROMPT "SQL>";
...
tracepath=`${sqlpluscmd} -S "/as sysdba" <<EOF 2>/dev/null
set heading off feedback off pagesize 0 linesize 200;
set verify off echo off termout off trimout on trimspool on ;
set time off timing off SQLPROMPT "SQL>";
