...
while getopts 'p:a' OPT; do
    case ${OPT} in
      p)
        __oraprofile="${OPTARG}"
        if [ -f "$__oraprofile" ]; then
            source $__oraprofile
        else
            echo $__oraprofile not found.
            exit 1
        fi
        ;;
      a)
...
[ -z "${__oraclehome}" ] && {
    [ -z "$__oraprofile" ] && { echo "If ORACLE_HOME not be set, use -p my.profile to set."; exit 1; }
}
...
processlist=$(ps -ef | awk '$1=="'$__runas'"{print}' | grep "ora_ckpt_" | egrep -v "grep|ASM|MGMTDB|+APX" | awk '{print $NF}' | sed 's/^ora_ckpt_//'|grep -v '/')
for p in $@
do
    [ "$p" = "normal" ] && processlist=$(ps -ef | grep "ora_ckpt_" | egrep -v "grep|ASM|MGMTDB|+APX" | awk '{print $NF}' | sed 's/^ora_ckpt_//'|grep -v '/')
done

[ -z "$processlist" ] && {
   psexist=NO
   echo "Oracle ora_ckpt_ process not exists or not run as current user, or try: sh $0 [ORACLE_HOME] normal"; exit 1;
} 

ipaddr=

which ip >/dev/null 2>&1
[ $? -eq 0 ] && ipaddr=$(ip a|grep -w inet|grep -v "127.0.0"|awk  '{print $2}'|head -1 | grep --color -o '\([0-9]\{1,3\}\.\)\{3\}[0-9]\{1,3\}')

[ -z "$ipaddr" ] && {
    which ifconfig >/dev/null 2>&1
    [ $? -eq 0 ] && ipaddr=$(ifconfig -a 2>/dev/null | grep --color -o '\([0-9]\{1,3\}\.\)\{3\}[0-9]\{1,3\}' | grep -v '127.0.0.1' | head -1)
}

[ -z "$ipaddr" ] && {
    which netstat >/dev/null 2>&1
    [ $? -eq 0 ] && ipaddr=$(netstat -ant | awk '{print $4}'| awk -F: '{print $1}' | grep -vE "^$|127.0.0.1|0.0.0.0" | grep --color -o '\([0-9]\{1,3\}\.\)\{3\}[0-9]\{1,3\}' | grep -v '127.0.0.1' | head -1)
}

[ -z "$ipaddr" ] && {
    ipaddr=$(cat /etc/hosts | grep -w $(hostname) | awk '{print $1}' | head -1)
}

[ ${__namewithip} = "yes" ] && {
__name="$(hostname | sed 's/\./%/g')_$(echo $ipaddr | sed 's/ //g' | sed 's/\./-/g')"
} || {
__name=$(hostname | sed 's/\./%/g')
}
...
echo OS UTC: $(date |awk '{print $5}')
OS_TY=$(cat /proc/version|cut -d "(" -f 4|cut -d " " -f 1)
if [ "$OS_TY" == "Red"  ]
then
...
echo "Kernel: $(uname -r)"
echo "Level: $(who -r | awk '{print $2}')"
echo "UptimeNumber: $(cat /proc/uptime | awk '{print $1/60}')"
echo "TimeZone: $(date | awk '{print $5}')"
}

_limits(){
cat /etc/security/limits.conf 2>/dev/null |grep -v "^$" |grep -v "^#" | awk '{print $1, $2, $3, $4}'
}
...
_fspct(){
df -m -P -t ext4 -t xfs -t btrfs -x iso9660 | grep -vE "^Filesystem|文件系统" | awk '{print $6","$1","$2/1024","$4/1024","$5}' | grep -v '^/proc'
}

_inodepct(){
df -i -P -t ext4 -t xfs -t btrfs -x iso9660 | grep -vE "^Filesystem|文件系统" | awk '{print $6","$1","$5}' | grep -v '^/proc'
}
...
_lo(){
echo "UEK version: "$(uname -r |awk  -F "." '{print $1}' 2>/dev/null)

mtu=
which ifconfig >/dev/null 2>&1
[ $? -eq 0 ] && mtu=$(ifconfig lo 2>/dev/null | grep mtu| awk '{print $NF}')

[ -z "$mtu" ] && {
    which netstat >/dev/null 2>&1
    [ $? -eq 0 ] && mtu=$(netstat -i 2>/dev/null | grep ^lo | awk '{print $2}')
}

[ -z "$mtu" ] && mtu="N/A"
echo "Current config: "$mtu

echo "Config file: "$(cat /etc/sysconfig/network-scripts/ifcfg-lo 2>/dev/null|grep "MTU"|awk -F "=" '{print $2}')
}
...
_osparm(){
/sbin/sysctl -a 2>/dev/null | grep -E "kernel.shmall|kernel.shmmax|vm.swappines|vm.swappiness|vm.dirty_background_ratio|vm.dirty_ratio|vm.dirty_expire_centisecs|vm.dirty_writeback_centisecs |net.ipv4.ipfrag_high_thresh |net.ipv4.ipfrag_low_thresh|vm.min_free_kbytes|kernel.randomize_va_space|kernel.exec-shield" | awk -F "=" '{print "\""$1"\""",""\""$2"\""}'
cat /proc/meminfo| grep -E "MemTotal|MemFree|MemAvailable|Buffers|Cached|SwapCached|AnonHugePages|HugePages_Total|HugePages_Free|SwapTotal|SwapFree" |awk -F ":" '{print $1   $2}' |awk  '{print "\"" $1"\""",""\""$2"\""}'
echo '"transparent_hugepage",' '"'$(awk -F '\\[|\\]' '{print $2}' /sys/kernel/mm/transparent_hugepage/enabled)'"'
}

_netstat(){
which netstat >/dev/null 2>&1
[ $? -eq 0 ] && /bin/netstat -in 2>/dev/null |grep -v "Kernel Interface table"|grep -v "Iface"| grep -v "lo"|grep -v "no statistics available"|grep -i -v "usb"|awk '{print $1",",$2",",$(NF-7)",",$(NF-6)",",$(NF-5)",",$(NF-3)",",$(NF-2)",",$(NF-1)}'
}
...
EOF
` | sed 's/ //g')
#cluster=$(echo $cluster | sed 's/,/\n/g' | grep -v ^$ | awk -F'@' '{$3="'$__name'"; print $1"@"$2"@"$3}' | sed 's/$/,/g')
...
export __out=./Oracle.$sid.${__name}.$(date +%Y%m%d%H%M%S).eout
>./$__out
