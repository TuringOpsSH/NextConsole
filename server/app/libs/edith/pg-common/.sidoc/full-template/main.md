...
{{$pgSummary := JsCall "func.pgSummary" .problems}}
{{range $k, $v := $pgSummary}}
...
### 数据库大小
{{range $kk, $vv := $v}}
{{if eq $vv.id "cluster_info"}}

|数据库名称|大小|
|:--|:---|
|{{range $kkk, $vvv := $vv.raw.database_sizes}}{{$vvv.name}}|{{$vvv.size}}MiB|
{{end}}{{end}}{{end}}


### 数据库表的大小信息
{{range $kk, $vv := $v}}
{{if eq $vv.id "table_sizes"}}
|#|Table|Rows|Total size|Table size|Index(es) Size|
|:--|:---|:---|:---|:---|:---|
||TOTAL|{{$vv.raw.tables_data_total.row_estimate_sum}}|{{$vv.raw.tables_data_total.total_size_bytes_sum}}|{{$vv.raw.tables_data_total.table_size_bytes_sum}}|{{$vv.raw.tables_data_total.indexes_size_bytes_sum}}|
|{{range $kkk, $vvv := $vv.raw.tables_data}}{{$vvv.num}}|{{$vvv.table}}|{{$vvv.row_estimate}}|{{$vvv.total_size_bytes}}({{$vvv.total_size_percent}})|{{$vvv.table_size_bytes}}({{$vvv.table_size_percent}})|{{$vvv.indexes_size_bytes}}|
{{end}}{{end}}{{end}}
