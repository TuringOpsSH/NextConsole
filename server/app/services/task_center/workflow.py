...
def emit_workflow_status(params):
    ...
    if not all_user_clients:
        ...
        return
    from app.services.next_console.workflow import show_workflow_task_label
    all_user_clients = json.loads(all_user_clients)
    for client in all_user_clients:
        # print(f"client: {client}")
        if client.get('status') == 'connected':
            data = show_workflow_task_label(new_task)
...
@celery.task
def async_generate_chart_options(params):
    """
    异步执行助手指令
    :param params:
    :return:
    ...
    with app.app_context():
        from app.services.assistant_center.assistant_instruction import run_assistant_instruction
        chart_options = run_assistant_instruction(params).json.get("result")
        # 推送结果至前端
        all_user_clients = redis_client.get(params.get("user_id"))
        if not all_user_clients:
            return
        data = {
            "msg_id": params.get("msg_id"),
        ...
        for client in all_user_clients:
            if client.get('status') == 'connected':
                socketio.emit("update_chart_options", data, room=client.get('session_id'))
