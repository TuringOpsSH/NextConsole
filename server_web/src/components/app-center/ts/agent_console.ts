...
import { addAppMessages, search_messages, update_session } from '@/api/next-console';
import {
  addAgentAppQa,
  AgentAppMsgFlow,
  CurrentAgent,
...
import { socket } from '@/components/global/web_socket';
import { running_questions, user_batch_size } from '@/components/next-console/messages-flow/console_input';
import { llm_instance_queue } from '@/components/next-console/messages-flow/llm_instance';
import {
...
export async function askAgentQuestion() {
  ...
  const userMsgContent = consoleInput.value.replace(/\n/g, '\n\n');
  // 生成问答对占位
  const new_qa_item_idx = AgentAppMsgFlow.value.length;
  const new_qa_item = <msg_queue_item>{
    qa_id: null,
    qa_status: null,
    qa_topic: null,
    qa_value: {
  ...
  const new_qa = await addAgentAppQa();
  if (!new_qa?.qa_id) {
    return;
  }
  ...
  // 如果启动了rag，则打开工作流占位符
  if (
    CurrentAgentAppSession.value.session_search_engine_switch ||
  ...
  const data = await addAppMessages(question_params, null);
  if (data.error_status) {
    return;
  }
  const msg_parent_id = data.result.msg_id;
  const msg_abort_controller = new AbortController();
  // 更新数据
  ...
  try {
    const data = await addAppMessages(params, msg_abort_controller.signal);
    // 处理流式响应来更新消息
    const reader = data.body.getReader();
    let chunk = await reader.read();
    while (!chunk.done) {
      // 当第一个字符出现时，获取参考文献，关闭工作流展示区
      if (!AgentAppMsgFlow.value[new_qa_item_idx].qa_value.answer[msg_parent_id][0].msg_content) {
        getTargetAgentReference(msg_parent_id);
        AgentAppMsgFlow.value[new_qa_item_idx].qa_workflow_open = false;
      }
      let jsonDataStr = new TextDecoder('utf-8').decode(chunk.value);
  ...
  console.log(1, user_batch_size.value);
  user_batch_size.value -= 1;
  // 完成后更新回答结果的id
  const data2 = await search_messages({
    qa_id: [CurrentAgentAppQas.value[0].qa_id]
  });
  if (!data2.error_status) {
    try {
      AgentAppMsgFlow.value[new_qa_item_idx].qa_value.answer[msg_parent_id][0].msg_id =
  ...
  add_copy_button_event();
  // 更新问题状态
  const idx = running_questions.value.indexOf(target_question);
...
export async function showSupDetail(item: msg_queue_item, event) {
  if (
    event.target.tagName.toLowerCase() === 'sup' ||
    ...
    const question_id = item.qa_value.question[0].msg_id;
    const reference_list = CurrentAgentReference.value?.[question_id]; // 获取 <sup> 的内容
...
export async function startRecording(src) {
...
export async function stopRecording(src) {
  // event.preventDefault();
  const userInfoStore = useUserInfoStore();
  if (isRecording.value) {
...
export async function switchLlmInstance(item: ILLMInstance) {
  // 切换llm_instance
  // welcome界面
  ...
  const data = await update_session(params);
  if (!data.error_status) {
    CurrentAgentAppSession.value.session_llm_code = item.llm_code;
    modelListRef.value?.hide();
    ElMessage.success('切换成功!');
