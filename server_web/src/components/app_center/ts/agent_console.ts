...
import {add_copy_button_event, splitMarkdown, tooltipStyle} from "@/components/next_console/messages_flow/message_flow";
import {msg_item, msg_queue_item, reference_item, running_question_meta} from "@/types/next_console";
import { getCurrentTimestamp } from "@/utils/base";
import {addAppMessages, search_messages, update_session} from "@/api/next_console";
import {
  running_questions,
  user_batch_size
...
import {llm_instance_queue} from "@/components/next_console/messages_flow/llm_instance";
...
export async function askAgentQuestion() {
  ...
  let data = await addAppMessages(question_params, null)
  ...
  try {
    ...
    let data = await addAppMessages(params, msg_abort_controller.signal)
    // 处理流式响应来更新消息
      ...
      for (let line of lines) {
        if (line.startsWith('data:')) {
          ...
          AgentAppMsgFlow.value[
            ...
            msg_parent_id][0].msg_content += jsonData?.choices[0].delta?.content;
