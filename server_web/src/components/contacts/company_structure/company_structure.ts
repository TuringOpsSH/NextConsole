...
export async function init_company_structure_tree(){
    ...
    if (user_info.value.user_account_type != "企业账号"){
        console.log('非企业账号，无法查看公司结构', user_info.value)
        return
    }
    // 获取公司领导人
    let leaders = await get_colleague_list({is_root: true})
...
export async function get_company_structure_tree(node: Node, resolve: (data: Tree[]) => void){
    ...
    let data: Tree[] = []
    // 获取部门下的同事
    let leaders = await get_colleague_list({department_id: node.data?.department_id})
    if (!leaders.error_status){
        for (let leader of leaders.result){
            let leader_data = {
                label: leader.user_name || leader?.user_nick_name,
...
export async function set_current_item_type(node:Node, data:Tree){
    ...
    if (data.structure_type === 'department'){
        let department_detail_res = await get_department_info({department_id: data.department_id})
