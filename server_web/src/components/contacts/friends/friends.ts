...
import {
    ...
    delete_friend,
    get_friend_list,
    get_friend_request_history, reject_friend_request,
    search_friends,
    search_stranger
} from "@/api/contacts";
import {ElMessage} from "element-plus";
import {init_friend_request_cnt} from "@/components/contacts/contacts_panel/contacts_panel";
...
export async function confirm_delete_friend(){
    ...
    let res = await delete_friend(params)
    if (!res.error_status){
...
export async function search_new_friends_list(){
    ...
    let res = await search_stranger(params)
    if (!res.error_status && res.result){
        search_new_friends.value = [res.result]
    }
...
export async function send_add_friend_request(){
    ...
    let res = await  add_friend(params)
    if (!res.error_status){
        ElMessage.success('添加好友请求已发送！')
        let params = {
            new_friend_email : new_friend_search_keyword.value
        }
        let new_friend = await search_stranger(params)
...
export async function enter_history_model(){
    ...
    friend_loading.value = true
    let history_friends_res = await get_friend_request_history({})
...
export async function handle_accept(friend_request:FriendRelation){
    ...
    let res = await accept_friend_request(params)
    if (!res.error_status){
...
export async function handle_reject(friend_request:FriendRelation){
    ...
    let res = await reject_friend_request(params)
    if (!res.error_status){
