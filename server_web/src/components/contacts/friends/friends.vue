...
const dialogWidth = phoneView ? '90vw' : '600px';
</script>
...
<template>
  <el-container>
    <el-header style="padding: 0 !important">
      <div id="friend_area">
        <el-text id="friend_area_title"> 外部联系人 </el-text>
      </div>
    </el-header>
    <el-container v-loading="friend_loading" style="margin-top: 4px">
      <el-aside style="border-right: 1px solid #e9ebf0" :width="friends_width">
        <el-scrollbar>
          <div id="friend_list_area">
            <div v-show="current_model != 'add'" id="friend_search_area">
            ...
            <div v-show="current_model == 'add'" id="friend_search_area">
              <div class="std-middle-box">
                <el-input
                  v-model="new_friend_search_keyword"
                  placeholder="搜索新的朋友 by 邮箱/手机"
                  clearable
                  style="width: 100%"
            ...
            <div
              v-for="(item, idx) in current_friend_list"
              v-show="current_model == 'list' || current_model == 'history'"
              :key="idx"
              class="friend-type-box"
              :class="{ 'friend-type-box-active': current_friend?.user_id == item.user_id }"
              @click="set_current_friend(item)"
            >
              <div class="std-middle-box">
                <el-avatar v-if="item?.user_avatar" :src="item?.user_avatar" style="background-color: white" />
                <el-avatar v-else style="background-color: #d1e9ff">
            ...
            <div
              v-for="(item, idx) in current_search_friends"
            ...
            <div
              v-for="(item, idx) in search_new_friends"
      ...
      <el-main style="height: calc(100vh - 100px); padding: 0 !important">
        <div v-if="phoneView" id="back-friends" style="margin-left: 30px">
          <el-button v-if="phoneView" type="text" @click="back_friends_list"> 返回 </el-button>
        </div>
        <div v-show="current_model != 'history'" id="show-info-area">
          <div v-if="current_friend?.user_id" id="show-info-box">
            <div id="show-info-head">
              ...
              <div class="std-middle-box">
                <el-avatar
                  :src="current_friend?.user_avatar"
            ...
            <div id="show-info-foot">
              ...
              <div class="std-middle-box">
                <el-button type="danger" @click="show_delete_confirm_flag = true"> 删除 </el-button>
              </div>
            </div>
          </div>
          <div v-else-if="current_new_friend?.user_id" id="show-info-box">
            <div id="show-info-head">
              <div id="show-info-head-left">
                <div class="std-middle-box" style="gap: 6px">
                  <div class="std-middle-box">
              ...
              <div class="std-middle-box">
                <el-avatar
                  :src="current_new_friend?.user_avatar"
                  style="width: 60px; height: 60px; background-color: white"
                />
              </div>
            </div>
            ...
            <div id="show-info-foot">
              <div
                v-show="!current_new_friend?.rel_status || current_new_friend?.rel_status?.rel_status >= 2"
                class="std-middle-box"
              >
                <el-button type="primary" @click="send_add_friend_request()"> 添加好友 </el-button>
              </div>
              <div v-show="current_new_friend?.rel_status?.rel_status == 1" class="std-middle-box">
                <el-button type="primary"> 发送信息 </el-button>
              </div>
              <div
                v-show="
                  current_new_friend?.rel_status?.rel_status == -1 &&
                  current_new_friend?.rel_status?.friend_id == userInfoStore.userInfo?.user_id
                "
                class="std-middle-box"
              >
                <el-button disabled type="info"> 已拒绝 </el-button>
              </div>
              <div
                v-show="
                  current_new_friend?.rel_status?.rel_status == -1 &&
                  current_new_friend?.rel_status?.user_id == userInfoStore.userInfo?.user_id
                "
                class="std-middle-box"
              >
                <el-button disabled type="danger"> 被拒绝 </el-button>
              </div>
              <div v-show="current_new_friend?.rel_status?.rel_status == 0" class="std-middle-box">
                <el-button disabled type="info"> 申请中 </el-button>
              </div>
            </div>
          </div>
          <div v-else id="show-info-box">
            <el-text> 请选择一个好友查看详细信息 </el-text>
          </div>
        </div>
        <div v-show="current_model == 'history'" id="show-history-area">
          <el-scrollbar>
            <div id="show-history-box">
              <div v-for="(record, idx) in history_friends" :key="idx" class="history-record-box">
                <div class="std-middle-box">
                ...
                <div class="std-middle-box" style="min-width: 60px">
                  <el-text v-show="record.rel_status?.rel_status == 1"> 已添加 </el-text>
                  <el-text v-show="record.rel_status?.rel_status == -1"> 已拒绝 </el-text>
                  <el-button
                    v-show="
                      record.rel_status?.rel_status == 0 &&
                      record.rel_status?.friend_id == userInfoStore.userInfo.user_id
                    "
                    type="success"
                    @click="handle_accept(record.rel_status)"
                  >
                    接受
                  </el-button>
                  <el-button
                    ...
                    "
                    type="warning"
                    @click="handle_reject(record.rel_status)"
                  >
                    拒绝
                  </el-button>
                  <el-text
                    v-show="
                      record.rel_status?.rel_status == 0 && record.rel_status?.user_id == userInfoStore.userInfo.user_id
                    "
                  >
                    申请中
                  </el-text>
                  <el-text v-show="record.rel_status?.rel_status > 1"> 已删除 </el-text>
                </div>
              </div>
            </div>
          </el-scrollbar>
    ...
    <el-dialog v-model="show_delete_confirm_flag" title="删除好友" :width="dialogWidth">
      <el-result
        icon="warning"
        title="确定删除好友吗？"
