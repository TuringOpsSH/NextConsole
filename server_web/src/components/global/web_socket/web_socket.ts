...
import {
    current_resource_list as current_resource_shortcut_list
} from "@/components/resource/resource_shortcut/resource_shortcut";
import {recommend_question_item, workflow_task_item, workflow_task_map} from "@/types/next_console";
import {msg_flow, msg_recommend_question, qa_workflow_map} from "@/components/next_console/messages_flow/message_flow";
import {session_history_top5} from "@/components/next_console/messages_flow/sessions";
import {Friend} from "@/types/contacts";
import {current_friend_request_cnt} from "@/components/contacts/contacts_panel/contacts_panel";
import {SystemNotice} from "@/types/users";
import {unread_system_notice} from "@/components/global/next_console_global_aside/menu_data";
import {currentPathTree} from "@/components/resource/resource_view/resource_viewer";
import {consoleInput} from "@/components/app_center/ts/agent_console";
import {consoleInputRef, user_input} from "@/components/next_console/messages_flow/console_input";
...
import {
    ...
    QAWorkFlowMap,
    sendMessageToParent
} from "@/components/app_center/ts/agent-app";
...
export async function initSocket(){
    ...
    socket.value.on('updateRefStatus',  (data) => {
        // console.log('update_ref_status',data)
        update_ref_status(data)
    });
    // 工作流更新
    ...
    // 更新站内信
    socket.value.on('new_system_notice',  (data:SystemNotice) => {
        // console.log('update_system_msg',data)
        update_system_notices(data)
    })
    // 更新资源名称
...
export function updateUserInput(newQuestion:string){
    ...
    else if  (router.currentRoute.value.name === 'next_console_welcome_home' ||
      router.currentRoute.value.name === 'message_flow'
    ){
        user_input.value += newQuestion
    }
}
export function update_ref_status(data:ResourceItem[]){
    // 根据传入的数据更新ref状态
    // data: 需要更新的资源对象列表
...
export function update_workflow_item_status(data:workflow_task_item, targetWorkflowMap:workflow_task_map){
    ...
    if(data.task_type == '会话命名') {
        // 更新会话列表中会话主题字段
        for (let history of session_history_top5.value){
...
export function update_recommend_question_lists(data:recommend_question_item[]){
    for (let recommend_question of data){
        if (!msg_recommend_question.value?.[recommend_question.msg_id]){
            msg_recommend_question.value[recommend_question.msg_id] = [
                recommend_question
            ]
        }
        else {
...
export function update_system_notices(data:SystemNotice){
    unread_system_notice.value.unshift(data)
}
