...
import {
  ...
  get_workflow_progress_batch as getWorkflowProgressBatch,
...
const currentSession = reactive<ISessionItem>({ id: 0 });
...
function addQuote(attachments: ISessionAttachment[]) {
...
async function getLastedWorkflow() {
  // 获取最新的工作流
  let params = {
    qa_ids: []
  };
  for (let i = 0; i < msgFlow.value.length; i++) {
    params.qa_ids.push(msgFlow.value[i].qa_id);
  }
  let res = await getWorkflowProgressBatch(params);
  if (!res.error_status) {
...
function clickRecommendQuestion(recommendQuestion: IRecommendQuestionItem) {
  // 点击推荐问题,直接提问并更新点击
  updateRecommendQuestionAPI({ recommend_question_id: recommendQuestion.id });
...
async function beginAnswer(data) {
  ...
  // console.log('add', msgFlow.value, currentSession.id)
...
<template>
  <el-container>
    <el-main class="msg-main" @dragover.prevent="handleDragOver">
        ...
        <el-row style="width: 100%" :style="{ height: msgFlowHeight }">
          ...
          <el-col :span="20" :xs="22">
            <div id="message-flow-box" ref="msgFlowBoxRef">
              ...
              >
                <div
                  class="msg-flow-question-box"
                  :class="[`question-layout-${userConfigStore.userConfig.workbench.message_layout}`]"
                  ...
                  <div class="msg-flow-question-content">
                      ...
                      <div class="question-button-box">
                        <div class="quote-right-container">
                          <el-tooltip content="引用" placement="top">
                            <SvgIcon
                              v-if="item.qa_value.question[0]?.attachment_list?.length > 0"
                              name="quote-right"
                              :width="16"
                              :height="16"
                    ...
                    <div class="attachment-area">
                      <div v-for="(attachment, idx2) in item.qa_value.question[0]?.attachment_list" :key="idx2">
                        <div v-if="attachment.resource_type == 'image'" class="attachment-item attachment-item-img">
                          <el-image
                            :zoom-rate="1.2"
                            :max-scale="7"
                            :min-scale="0.2"
                ...
                <div
                  class="msg-flow-answer-box"
                  :class="[`answer-layout-${userConfigStore.userConfig.workbench.message_layout}`]"
      ...
      <div v-show="showSupDetail" id="sup_detail_box" :style="tooltipStyle" @mouseleave="showSupDetail = false">
