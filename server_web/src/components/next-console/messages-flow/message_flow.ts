...
import { ask_question, running_questions, user_input } from '@/components/next-console/messages-flow/console_input';
import {
  msg_queue_item,
...
export async function get_target_reference(msg_id: number) {
  ...
  if (!res.error_status) {
    ...
    for (const msg_item of msg_flow.value) {
...
export function add_copy_button_event() {
  const copy_button_list = document.getElementsByClassName('answer-code-copy');
  for (let i = 0; i < copy_button_list.length; i++) {
    const button = copy_button_list[i] as HTMLElement;
    if (button.dataset.clickListener === 'true') {
      continue;
    }
    copy_button_list[i].addEventListener('click', function () {
      const code = (this.parentNode.nextElementSibling && this.parentNode.nextElementSibling.innerText) || '';
  ...
  try {
    ...
    supTags.forEach(function (supTag) {
      // 获取 <sup> 标签下的所有 <a> 标签
      const aTags = supTag.querySelectorAll('a');
...
export function copy_question(item: msg_queue_item) {
  const content = item?.qa_value?.question?.[0]?.msg_content;
  if (!content) {
    return;
  }
...
export function copy_answer(item: msg_queue_item) {
  ...
  const content = item?.qa_value?.answer?.[msg_parent_id]?.[0]?.msg_content;
  if (!content) {
    return;
  }
  // 去除 <sup> 标签及其内容
...
export function get_msg_item_answer_create_time(item: msg_queue_item) {
  try {
    const msg_parent_id = item?.qa_value?.question?.[0].msg_id;
...
export async function add_like(item: msg_queue_item) {
  ...
  const answer_id = item.qa_value.answer[question_id][0]?.msg_id;
  if (!answer_id) {
    ElMessage.info({
...
export async function show_sup_detail_fn(item: msg_queue_item, event) {
  if (
    ...
    const reference_list = msg_flow_reference.value?.[question_id]; // 获取 <sup> 的内容
...
export async function get_lasted_workflow() {
  ...
  const res = await get_workflow_progress_batch(params);
  if (!res.error_status) {
