...
import {Microphone, Picture as IconPicture, VideoPause} from '@element-plus/icons-vue';
...
import ResourcesSearch from '@/components/next_console/ResourcesSearch.vue';
import { isRecording, startRecording, stopRecording } from '@/components/app_center/ts/agent_console';
import {
  ElMessage,
  ElNotification,
  genFileId,
...
import { running_question_meta, session_item } from '@/types/next_console';
import {
  add_messages,
...
import { LLMInstance, SearchResourceType } from '@/types/config_center';
...
const llmInstanceQueue = ref<LLMInstance[]>([]);
...
const availableSearchResourceType = ref<SearchResourceType[]>([
  {
...
async function switchLlmInstance(item: LLMInstance) {
  // 切换llm_instance
  // welcome界面
...
async function switchOnAiSearch() {
  ...
  for (let i = 0; i < availableSearchResourceType.value.length; i++) {
    availableSearchResourceType.value[i].resource_active = availableSearchResourceType.value[i].resource_type == currentSession.session_search_engine_resource_type;
  }
  await nextTick(() => {
    if (window.innerWidth >= 768) {
...
function checkImgSupportStatus(imgResource: ResourceItem) {
  ...
  // 每张图片不能超过20Mb
  if ( imgResource.resource_name.endsWith('.png') || imgResource.resource_name.endsWith('.jpg')
      || imgResource.resource_name.endsWith('.jpeg') || imgResource.resource_name.endsWith('.webp')
  ) {
...
async function uploadImgSuccess(response: any, uploadFile: UploadFile, uploadFiles: UploadFiles) {
  ...
  for (let i = 0; i < llmInstanceQueue.value.length; i++) {
    if (llmInstanceQueue.value[i]?.llm_type == 'qwen-vl-max') {
  ...
  for (let i = 0; i < upload_file_task_list.value.length; i++) {
    if (upload_file_task_list.value[i].task_status  == 'pending'
...
function checkFileSupportStatus(fileResource: ResourceItem) {
  let allSupportFormats = [
    ...
    "htm", "html",
    "log","syslog","audit","wevt","kmsg","access","cfg","ini","service","rules","policy","rrd","tsd","metrics","stats","pcap","flow","sflow","pdns","crontab","fstab","grub","conf","ovf","qcow2","yaml","toml","hash","sig","gpg","p12","diff","snap","tar","manifest"
  ];
...
async function uploadFileSuccess(response: any, uploadFile: UploadFile, uploadFiles: UploadFiles) {
  ...
  let finishFlag = true;
  for (let i = 0; i < upload_file_task_list.value.length; i++) {
    if (upload_file_task_list.value[i].task_status  == 'pending' || upload_file_task_list.value[i].task_status  == 'uploading') {
      finishFlag = false;
      break;
    }
  }
