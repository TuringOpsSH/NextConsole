...
import {add_resource_tags, delete_resource_tags, search_resource_tags, update_resource_tags} from '@/api/resource-api';
...
import {
  current_resource_tags, current_resource_types,
...
function initSystemTags(resourceTypeMax: number = 4) {
  ...
  for (let systemTag of system_tags) {
    if (systemTag.tag_type == 'recent') {
      //@ts-ignore
      panel_recent_shortcuts.value.push(systemTag);
    } else if (systemTag.tag_type == 'resource_type') {
      //@ts-ignore
      panel_system_labels.value.push(systemTag);
      idx += 1;
...
async function loadAllTags() {
  ...
  for (let userTag of panel_user_labels.value) {
...
<template>
  ...
  <div id="panel_recent_area">
    ...
    <div v-show="showLabelArea && !load_all_flag" id="panel_label_foot" @click="loadAllTags">
    ...
    >
      ...
      <el-form-item prop="tag_color" label="标签颜色">
        <el-select v-model="newTagFormData.tag_color" :default-first-option="true">
          ...
          <el-option
            v-for="(tag_color, idx) in tag_color_list"
            :key="idx"
