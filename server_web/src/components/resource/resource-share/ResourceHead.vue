...
import { get_resource_object_path as getResourceObjectPath } from '@/api/resource-api';
...
const currentPathTree = ref<IResourceItem[]>([]);
const showDirMetaFlag = ref(false);
...
const searchPlaceholder = computed(() => {
  ...
  return lastPath?.length >= 8 ? `在 ${lastPath.slice(0, 8)}... 中搜索` : `在 ${lastPath} 中搜索`;
});
...
async function getShareParentResourceList(newId: number) {
  ...
  const res = await getResourceObjectPath(params);
  if (!res.error_status) {
    for (const item of res.result.data) {
      if (item?.resource_parent_id) {
        currentPathTree.value.push(item);
      }
    }
  }
}
watch(
  () => props.resourceId,
  async newValue => {
    getShareParentResourceList(newValue);
  }
);
</script>
