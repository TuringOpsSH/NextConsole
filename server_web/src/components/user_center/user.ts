...
import {PointTransaction, Users} from "@/types/users";
import {ElMessage, FormRules} from "element-plus";
import {getInfo, getToken, login_out} from "@/utils/auth";
import {invite_view_id, valid_user_account} from "@/components/user_center/login";
import {
    bind_new_phone_api, close_user, list_point_transaction,
    refresh_invite_code,
    reset_new_email,
    valid_new_phone_api,
    valid_reset_email_code
...
export function initWxLogin() {
    ...
    let state = "login"
    if (invite_view_id.value !== null && invite_view_id.value !== undefined && invite_view_id.value !== ""){
        state = "login_" + invite_view_id.value
...
// 邀请码相关
export const current_invite_type = ref('invite_link')
export const current_invite_url = ref('')
export const show_invite_dialog_flag = ref(false)
export async function show_invite_dialog(){
    show_invite_dialog_flag.value = true
...
export function copy_invite_link(){
    Clipboard.copy(current_invite_url.value.trim())
    ElMessage({
        message: '复制成功',
        type: 'success',
        duration: 2000
...
export const refresh_confirm_flag = ref(false)
export async function refresh_invite_link(){
...
// 注销账号
export const close_account_flag = ref(false)
export async function close_user_account(){
    let res = await close_user({
    ...
    if (!res.error_status){
        ...
        close_account_flag.value = false
        login_out()
    }
}

// 个人积分账户
export const show_account_transaction = ref(false)
export const account_transaction_loading = ref(false)
export const account_transaction_data = ref<PointTransaction[]>([])
export const current_page_num = ref(1)
export const current_page_size = ref(10)
export const current_transaction_cnt = ref(0)
export async function show_account_transaction_dialog(){
    show_account_transaction.value = true
    account_transaction_loading.value = true
    user_info.value = await getInfo(true)
    let res = await list_point_transaction({
        page_num: current_page_num.value,
        page_size: current_page_size.value
    })
    if (!res.error_status) {
        account_transaction_data.value = res.result.data
        current_transaction_cnt.value = res.result.total
    }
    account_transaction_loading.value = false
}
export async function handleSizeChange(val: number){
    current_page_size.value = val
    show_account_transaction_dialog()
