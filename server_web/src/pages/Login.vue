...
import { validInviteApi } from '@/api/contacts';
...
async function validInvite() {
  if (LocalInviteViewId.value) {
    let res = await validInviteApi({
      invite_id: LocalInviteViewId.value
    });
    if (!res.error_status && res.result?.valid && !res.result?.homepage) {
      inviteModel.value = true;
    }
  }
}
...
onMounted(async () => {
  LocalLoginType.value = props.loginType;
  LocalInviteViewId.value = props.inviteViewId;
  if (LocalInviteViewId.value) {
    await validInvite();
  }
  // 动态加载 wxLogin.js 脚本
